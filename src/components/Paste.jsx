...
import { removeFromPastes, addToPastes } from "../redux/pasteSlice"; // Ensure your addToPastes action is imported
import { FormatDate } from "../utlis/formatDate";

const Paste = () => {
  const allPastes = useSelector((state) => state.paste.pastes); // Fetch all pastes
  const dispatch = useDispatch();
  const [searchTerm, setSearchTerm] = useState(""); // State to hold the search term
  const [filteredPastes, setFilteredPastes] = useState([]); // State for filtered pastes

  useEffect(() => {
    // Filter pastes by search term
    const searchFiltered = allPastes.filter(
      (paste) =>
        paste.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        paste.content.toLowerCase().includes(searchTerm.toLowerCase())
    );

    setFilteredPastes(searchFiltered);
  }, [allPastes, searchTerm]); // Run whenever pastes or searchTerm change

  // Function to handle adding a new paste (update as per your logic)
  const handleAddPaste = (paste) => {
    if (!paste.title.trim() || !paste.content.trim()) {
      // Show toast error if title or content is empty
      toast.error("Title and content cannot be empty.");
      return; // Prevent adding invalid paste
    }
    dispatch(addToPastes(paste)); // Add valid paste to Redux state
